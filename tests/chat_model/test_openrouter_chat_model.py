...
from magentic.chat_model.openrouter_chat_model import (
    OpenRouterChatModel,
    async_message_to_openrouter_message,
    message_to_openrouter_message,
)
...
message_to_openrouter_message_test_cases = [
    (
...
@pytest.mark.parametrize(
    ("message", "expected_openrouter_message"), message_to_openrouter_message_test_cases
)
def test_message_to_openrouter_message(message, expected_openrouter_message):
    assert message_to_openrouter_message(message) == expected_openrouter_message


async_message_to_openrouter_message_test_cases = [
    *message_to_openrouter_message_test_cases,
    (
        AssistantMessage(
            AsyncParallelFunctionCall(
...
@pytest.mark.parametrize(
    ("message", "expected_openrouter_message"),
    async_message_to_openrouter_message_test_cases,
)
async def test_async_message_to_openrouter_message(
    message, expected_openrouter_message
):
    assert (
        await async_message_to_openrouter_message(message)
        == expected_openrouter_message
    )


def test_message_to_openrouter_message_user_image_message_bytes_jpg(image_bytes_jpg):
    image_message = UserMessage([ImageBytes(image_bytes_jpg)])
    assert message_to_openrouter_message(image_message) == snapshot(
        {
            "role": "user",
            "content": [
                {
...
def test_message_to_openrouter_message_user_image_message_bytes_png(image_bytes_png):
...
def test_message_to_openrouter_message_raises():
    ...
    with pytest.raises(NotImplementedError):
        message_to_openrouter_message(CustomMessage("Hello"))

    @message_to_openrouter_message.register
    def _(message: CustomMessage) -> ChatCompletionMessageParam:
        return {"role": "user", "content": message.content}

    assert message_to_openrouter_message(CustomMessage("Hello")) == {
        "role": "user",
        "content": "Hello",
    }
