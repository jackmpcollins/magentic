...
class OpenRouterMessageRole(Enum):
    ASSISTANT = "assistant"
    SYSTEM = "system"
    TOOL = "tool"
    USER = "user"
...
@singledispatch
def message_to_openrouter_message(message: Message[Any]) -> ChatCompletionMessageParam:
    """Convert a Message to an OpenRouter message."""
    # TODO: Add instructions for registering new Message type to this error message
    raise NotImplementedError(type(message))


@singledispatch
async def async_message_to_openrouter_message(
    message: Message[Any],
) -> ChatCompletionMessageParam:
    """Async version of `message_to_openrouter_message`."""
    return message_to_openrouter_message(message)


@message_to_openrouter_message.register(_RawMessage)
...
@message_to_openrouter_message.register
def _(message: SystemMessage) -> ChatCompletionMessageParam:
    return {"role": OpenRouterMessageRole.SYSTEM.value, "content": message.content}


@message_to_openrouter_message.register(UserMessage)
def _(message: UserMessage[Any]) -> ChatCompletionUserMessageParam:
    if isinstance(message.content, str):
        return {"role": OpenRouterMessageRole.USER.value, "content": message.content}
    if isinstance(message.content, Iterable):
        ...
        for block in message.content:
            if isinstance(block, str):
                ...
                raise TypeError(msg)
        return {"role": OpenRouterMessageRole.USER.value, "content": content}
    msg = f"Invalid content type: {type(message.content)}"
    raise TypeError(msg)


@message_to_openrouter_message.register(UserImageMessage)
def _(message: UserImageMessage[Any]) -> ChatCompletionUserMessageParam:
    ...
    return {
        "role": OpenRouterMessageRole.USER.value,
...
@message_to_openrouter_message.register(AssistantMessage)
def _(message: AssistantMessage[Any]) -> ChatCompletionMessageParam:
    if isinstance(message.content, str):
        return {
            "role": OpenRouterMessageRole.ASSISTANT.value,
...
@async_message_to_openrouter_message.register(AssistantMessage)
...
@message_to_openrouter_message.register(ToolResultMessage)
def _(message: ToolResultMessage[Any]) -> ChatCompletionMessageParam:
    ...
    return {
        "role": OpenRouterMessageRole.TOOL.value,
...
def _add_missing_tool_calls_responses(
    ...
    This is required by OpenRouter's API.
...
class OpenRouterStreamParser(StreamParser[ChatCompletionChunk]):
...
class OpenRouterStreamState(StreamState[ChatCompletionChunk]):
    """Tracks the state of the OpenRouter model output stream.
...
class OpenRouterChatModel(ChatModel):
    ...
    def __init__(
        ...
        *,
        ...
        if not self._api_key:
            _error_message = "No API key provided"
            raise openai.OpenAIError(_error_message)
        ...
        self._async_client = openai.AsyncOpenAI(
            api_key=self._api_key, base_url=base_url
        )
    ...
    def require_parameters(self) -> bool:
        return self._require_parameters
    ...
    def _get_parallel_tool_calls(
        ...
        if not tools_specified:  # Enforced by OpenRouter API
            return openai.NOT_GIVEN
        ...
        *,
        ...
        response: Iterator[ChatCompletionChunk] = self._client.chat.completions.create(
            ...
            messages=_add_missing_tool_calls_responses(
                [message_to_openrouter_message(m) for m in messages]
            ),
    ...
    async def acomplete(
        ...
        *,
        ...
        response: AsyncIterator[
            ...
            model=self.model,
            messages=_add_missing_tool_calls_responses(
                [await async_message_to_openrouter_message(m) for m in messages]
            ),
            max_tokens=_if_given(self.max_tokens),
            seed=_if_given(self.seed),
            stop=_if_given(stop),
